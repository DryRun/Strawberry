...
class PHPRestSQL {
                ...
                if ($this->uid && count($primary) == count($this->uid)) { // get a row
                    ...
                    if ($resource) {
                        if ($this->db->numRows($resource) > 0) {
                            ...
                            while ($row = $this->db->row($resource)) {
                                // Get the entire row using the primary keys present in this row
                                $where = '';
                                foreach ($primary as $key => $pri) {
                                    $where .= $pri.' = \''.$row[$pri].'\' AND ';
                                }
                                $where = substr($where, 0, -5);
                                $resourceRow = $this->db->getRow($this->table, $where);
                                
                                if ($resourceRow) {
                                    if ($this->db->numRows($resourceRow) > 0) {
                                        while ($rowVals = $this->db->row($resourceRow)) {
                                            // Shove the row key=>value pairs onto an array
                                            $values = array();
                                            foreach ($rowVals as $column => $data) {
                                                $values[$column] = $data;
                                            }
                                            // Shove this row onto the array of rows
                                            $this->output['table'][] = $values;
                                        }
                                    }
                                }
    ...
    function generateResponseDataOld() {
		if ($this->extension) {
